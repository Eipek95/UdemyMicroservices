...
namespace FreeCourse.Services.Basket.Services
{
    public class BasketService : IBasketService
    {
        ...
        public async Task<Response<bool>> Delete(string userId)
        {
            ...
            return status ?
               Response<bool>.Success(StatusCodes.Status204NoContent)
               : Response<bool>.Fail("Basket could not found", StatusCodes.Status404NotFound);
        }

        public async Task<Response<BasketDto>> GetBasket(string userId)
        {
            var existBasket = await _redisService.GetDb().StringGetAsync(userId);//böyle bir keye sahip data varsa bana ver.json olarak verir
